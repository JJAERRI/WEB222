...
function toCamelCase(name, uppercase) {
  ...
  let parts = name.split('-').filter((part) => part.length > 0);

  if (parts.length === 0) {
    return '';
  }

  let camelCaseName = parts
    .map((part, index) => {
...
function createLinkTag(rel, href) {
  ...
  const relNeedsQuotes = specialCharsPattern.test(rel);
  const relAttribute = relNeedsQuotes ? `rel="${rel}"` : `rel=${rel}`;

  const hrefNeedsQuotes = specialCharsPattern.test(href);
  const hrefAttribute = hrefNeedsQuotes ? `href="${href}"` : `href=${href}`;

  return `<link ${relAttribute} ${hrefAttribute}>`;
}
...
function toDateString(value, format) {
  ...
  try {
    ...
    if (!validFormats.includes(format)) {
      throw new Error(
        'Invalid format string. Must be "YYYY/MM/DD", "DD/MM/YYYY", or "MM/DD/YYYY".'
      );
    }
...
function normalizeTime(value) {
  const twelveHourPattern = /^(\d{1,2}):(\d{2}):(\d{2})\s?(AM|PM)$/;
  const match12Hour = value.match(twelveHourPattern);
  if (match12Hour) {
    let [_, hours, minutes, seconds, period] = match12Hour;
  ...
  const match24Hour = value.match(twentyFourHourPattern);
  if (match24Hour) {
    let [_, hours, minutes, seconds] = match24Hour;
    hours = parseInt(hours, 10);
    minutes = parseInt(minutes, 10);
    seconds = parseInt(seconds, 10);
...
function formatTimes(...values) {
  const normalizedTimes = values.map(normalizeTime).filter((time) => time !== null);
  return `[${normalizedTimes.join(', ')}]`;
}
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  ...
  if (!resourceType || !resourceId) {
    throw new Error('resourceType and resourceId must be provided and non-empty');
  }
  ...
  if (extraData) {
    encodedExtraData = encodeURIComponent(extraData);
  }

  let apiEndpoint = `/${resourceType}/${resourceId}`;
  if (encodedExtraData) {
    apiEndpoint += `/${encodedExtraData}`;
  }
