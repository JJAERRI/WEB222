...
function speciesCoordinates(data) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    console.log(`"${observation.species_guess}" observed at coordinates (${observation.location})`);
  }
}
...
function speciesCoordinates3(data) {
  for (const observation of data.results) {
    const formattedLocation = observation.location.replace(',', ', ');
    console.log(`"${observation.species_guess}" observed at coordinates (${formattedLocation})`);
  }
}
...
function observationsByQualityGrade(data, qualityGrade) {
  const checkIsValid = (grade) => {
    if (grade === 'research' || grade === 'needs_id' || grade === 'casual' || grade === 'null') {
      return true;
    }
    throw Error();
  };

  qualityGrade = qualityGrade !== null ? [qualityGrade.toLowerCase()].filter(checkIsValid) : [null];
  return data.results.filter((element) => qualityGrade.includes(element.quality_grade));
}
...
function transformObservations(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    result.push(transformObservation(data.results[i]));
  }
  return result;
}
...
function getObservationsByTaxa(data, ...taxaNames) {
  ...
  return data.results.filter((element) => taxaNames.includes(element.taxon.iconic_taxon_name));
}

/**
 * let (results) = data // can also be let results = data.results
 *
 * if(taxaNames.length === 1) {
 * return results.taxon.iconic_taxan_name
 * }
 *
 */
...
function getObservationsByLocation(data, options = {}) {
  ...
  const latOpt = typeof options.lat === 'number' ? options.lat : undefined;
  const lngOpt = typeof options.lng === 'number' ? options.lng : undefined;
  if (latOpt && lngOpt) {
    return data.results.filter((element) => {
      const coordinates = element.geojson.coordinates;
      return coordinates[1] === latOpt && coordinates[0] === lngOpt;
    });
  }

  const latMin = options.lat ? options.lat.min : undefined;
  const latMax = options.lat ? options.lat.max : undefined;
  const lngMin = options.lng ? options.lng.min : undefined;
  const lngMax = options.lng ? options.lng.max : undefined;

  if (latMin && latMax && lngMin && lngMax) {
    return data.results.filter((element) => {
      const coordinates = element.geojson.coordinates;
      return (
        coordinates[1] > latMin &&
        coordinates[1] < latMax &&
        coordinates[0] > lngMin &&
        coordinates[0] < lngMax
      );
    });
  }
...
function getPlaceURLs(data) {
  ...
  data.results.forEach((element) => {
    results = results.concat(
      element.place_ids.map(
        (place_id) => `https://www.inaturalist.org/observations?place_id=${place_id}`
      )
    );
  });
  return results;
...
function getSpeciesObservations(data) {
  ...
  let result = {};
  data.results.forEach((element) => {
    if (result[element.taxon.name]) {
      result[element.taxon.name]++;
    } else {
      result[element.taxon.name] = 1;
    }
  });
  return result;
}
...
function extractSpeciesNames(data) {
  ...
  return data.results
    .map((element) => {
      const name = element.species_guess;
      if (!names[name]) {
        names[name] = true;
        return name;
      }
...
function extractSpeciesNames2(data) {
  // TODO
  const set = new Set();
  data.results.forEach((element) => set.add(element.species_guess));
